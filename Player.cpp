...
void Player::SetXCenter(float x) {
	xCenter = x;
}

void Player::SetYCenter(float y) {
	yCenter = y;
}
...
void Player::Init(Shader& shader, float colour[3], std::string filename) {
	//load png image
	int imageHeight = 0;
	int imageWidth = 0;
...
void Player::Render(Shader& shader, glm::mat4& ModelViewMatrix, glm::mat4& ProjectionMatrix) {
